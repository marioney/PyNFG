...
from pynfg_ros.utilities.utilities import iterated_input_dict
import warnings
import sys
...
class EwmaMcrl(object):
    ...
    def train_node(self, bn, level, setCPT=False):
        ...
        Q = np.zeros(shape).astype(np.float32)  # Qtable
        V = np.zeros(shape[:-1]).astype(np.float32)  # Value table
        Rseries = np.zeros(N).astype(np.float32)  # tracking average reward for plotting convergence
        np.seterr(invalid='ignore', divide='ignore')
        for n in xrange(N):
            sys.stdout.write('\r')
            sys.stdout.write('Iteration ' + str(n))
            sys.stdout.flush()
            indicaten = np.zeros(Q.shape).astype(np.float32)  # indicates visited mapairs
            ...
            # CPTsum = CPTsum[..., np.newaxis]
            # print CPTsum.shape
            # cpt_gpu = gpuarray.to_gpu(Game.bn_part[bn][0].CPT.astype(np.float32))
            # cptsum_gpu = gpuarray.to_gpu(CPTsum.astype(np.float32))
            # cpt_gpu /= cptsum_gpu
            # Game.bn_part[bn][0].CPT = cpt_gpu.get()
