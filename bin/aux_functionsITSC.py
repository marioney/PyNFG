    ...
    if pose_array.ndim is 1:
        ...
        for index_compare in range(0, rows):
            if np.array_equal(current_pose, pose_array[index_compare]):
                # print 'new_pose ' + str(current_pose) + 'in collision with car ' + str(index_compare)
                return True
            current_pose_1 = current_pose + np.array([0, 1])
            # print 'new_pose + 1 ' + str(current_pose_1)
            if np.array_equal(current_pose_1, pose_array[index_compare]):
                # print 'new_pose + 1 ' + str(current_pose_1) + 'in collision with car ' + str(index_compare)
                return True
            current_pose_2 = current_pose + np.array([0, 2])
            # print 'new_pose + 1 ' + str(current_pose_1)
            if np.array_equal(current_pose_2, pose_array[index_compare]):
                # print 'new_pose + 1 ' + str(current_pose_1) + 'in collision with car ' + str(index_compare)
                return True
...
def get_observations(current_status, nr_vehicles, vehicle_nr, north, east, west, blocked_north):
        ...
        if x == -1:
            if 0 < y <= my_vel:
                if current_velocities[index_node][1] > fr:
                    fr = current_velocities[index_node][1]
                    if fr > 1:
                        fr = 1
            if y == 0:
                rr = current_velocities[index_node][1]
                if rr > 1:
                    rr = 1
        ...
        if x == 0:
            if 0 < y <= my_vel:
                ...
                if current_velocities[index_node][1] > f:
                    f = current_velocities[index_node][1]
                    if f > 1:
                        f = 1
        ...
        if x == 1:
            if 0 < y <= my_vel:
                if current_velocities[index_node][1] > fl:
                    fl = current_velocities[index_node][1]
                    if fl > 1:
                        fl = 1
            if y == 0:
                rl = current_velocities[index_node][1]
                if rl > 1:
                    rl = 1
    ...
    observations = np.array([[fl, f, fr, rl, rr, vel]])
...
def get_observation_space(possible_speeds=[-100, 0, 1, 100]):
    possible_states = [np.array([[fl, f, fr, rl, rr, vel]])
