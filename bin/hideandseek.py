...
# starting locations
startingloc = np.array([[east, north-1], [west, north-1]])
...
def newloc(seekmove=np.array([0, 0]), hidemove=np.array([0, 0]), loc=startingloc):
    """
...
Dseek = DecisionNode('Dseek0', 'seeker', actionspace, parents=[Fseek], basename='Dseek', time=0)
# HIDER DecisionNode, Dhide
Dhide = DecisionNode('Dhide0', 'hider', actionspace, parents=[Fhide], basename='Dhide', time=0)

# STATE ROOT NODE, DeterNode F

paramsf = {'seekmove': Dseek, 'hidemove': Dhide, 'loc': Froot}
F = DeterNode('F0', newloc, paramsf, continuousf, space=statespace, basename='F', time=0)
...
G.bn_part['Dseek'][0].uniformCPT()
print G.bn_part['Dseek'][0].uniformCPT()
...
MCRL_solved.solve_game(setCPT=False)
...
#MCRL_solved.train_node('Dhide', 1, setCPT=False)
MCRL_solved.train_node('Dseek', 2, setCPT=True)
#MCRL_solved.train_node('Dseek', 2, setCPT=True)

valuedict = G.sample_timesteps(0, 10, basenames=['Froot', 'F', 'Dhide', 'Dseek'])
print (valuedict)
